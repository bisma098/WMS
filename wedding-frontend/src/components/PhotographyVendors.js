...
function PhotographyVendors() {
    ...
    useEffect(() => {
        fetch(`/available-vendors/${eventId}/photography`)
            .then((res) => res.json())
            .then((data) => {
                if (data.success) {
    ...
    const handleBooking = async (vendorId) => {
        try {
            const response = await axios.post('/book-photography-vendor', {
                eventId,
                photographyVendorId: vendorId,
            });
    ...
    return (
        <div className="available-vendors">
            <h2>Available Photography Vendors</h2>
