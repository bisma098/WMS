...
import './ManagerVendorsTable.css'; // New simple table CSS

function CateringVendorsManager() {
    ...
    const fetchVendors = async () => {
        try {
            const res = await axios.get('/catering-vendors');
            setVendors(res.data);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleAddVendor = async () => {
        try {
            await axios.post('/add-catering-vendor', newVendor);
            setNewVendor({
                name: '',
                contact_no: '',
                city: '',
...
export default CateringVendorsManager;
