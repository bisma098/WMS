...
function PhotographyVendorsManager() {
    ...
    const fetchVendors = async () => {
        try {
            const res = await axios.get('/photography-vendors');
            setVendors(res.data);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleDelete = async (vendorId) => {
        ...
        try {
            await axios.delete(`/delete-photography-vendor/${vendorId}`);
            fetchVendors();
        } catch (err) {
            console.error(err);
        }
    ...
    const handleAddVendor = async () => {
        try {
            await axios.post('/add-photography-vendor', newVendor);
            setNewVendor({
                name: '',
                contact_no: '',
                email: '',
    ...
    return (
        <div className="vendors-table-page">
            ...
            <h3>Add New Photography Vendor</h3>
...
export default PhotographyVendorsManager;
