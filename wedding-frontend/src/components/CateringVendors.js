...
function CateringVendors() {
    ...
    useEffect(() => {
        fetch(`/available-vendors/${eventId}/catering`)
            .then((res) => res.json())
            .then((data) => {
                if (data.success) {
                    setVendors(data.catering);
                } else {
    ...
    const handleBooking = async (vendorId) => {
        try {
            const response = await axios.post('/book-catering-vendor', {
                eventId,
                cateringVendorId: vendorId,
            });
    ...
    return (
        <div className="available-vendors">
            <h2>Available Catering Vendors</h2>
                ...
                <ul className="vendor-list">
                    {vendors.map((vendor) => (
                        <li key={vendor.Vendor_ID} className="vendor-card">
                            ...
                            <p><strong>Price per Person:</strong> Rs. {vendor.Cost}</p>
